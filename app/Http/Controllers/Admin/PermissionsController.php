...
{
    // Assuming you have a `sub_module` column or relationship in your `permissions` table
    $subModules = Permission::where('module', $module)
        ->select('id', 'sub_module')
        ->distinct()
        ->get();
...
public function filterPermissions(Request $request)
{
    $permissions = Permission::where('module', $request->module)
        ->where('sub_module', $request->sub_module)
        ->get();
    return response()->json($permissions);
}
