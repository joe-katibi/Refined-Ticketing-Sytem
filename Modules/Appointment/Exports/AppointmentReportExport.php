...
class AppointmentReportExport implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize
{
  protected $startDate;
  protected $endDate;
  ...
  public function map($row): array
  {
    switch ($this->reportType) {
      ...
      case 'assigned_team_productivity':
        return [
          $row->team_name ?? ($row->sub_type_name ?? $row->name),
          $row->total_assigned,
          $row->total_closed,
      ...
      default:
        ...
        return [
          ...
          $row->description_notes ?? 'N/A',
          $row->appointment_type_name ?? 'N/A',
          $row->status,
          $row->priority ?? 'N/A',
          $row->team_name ?? 'N/A',
          $row->sub_type_name ?? 'N/A',
          $row->closed_by_user ?? 'N/A',
          $row->final_reason_name ?? 'N/A',
          $row->created_at,
