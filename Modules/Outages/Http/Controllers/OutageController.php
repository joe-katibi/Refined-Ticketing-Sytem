...
class OutageController extends OutagesController
{
    ...
    public function store(Request $request)
    {
        ...
        // Generated BEFORE the transaction below on purpose: generateTicketNumber()
        // calls SequenceNumberService::next(), which commits its own increment
        // immediately. Calling it inside the transaction would mean a later
        // failure here rolls back that increment too — silently "returning" the
        // ticket number for the next outage created to hand out again.
            ...
            return response()->json([
                ...
                'ports' => $ports->map(function ($port) {
                    return [
                        ...
                        'status' => $port->status,
                        'display_name' => "Port {$port->pon_port_number}".($port->pon_port_type ? " ({$port->pon_port_type})" : ''),
                    ];
                }),
            ]);
        } catch (\Exception $e) {
    ...
    public function updateMyOutage(Request $request, Outage $outage)
    {
        ...
        if ($request->hasFile('attachments')) {
            foreach ($request->file('attachments') as $file) {
                // Generate the stored filename instead of trusting the client-supplied
                // original name in the storage path — an unsanitized name there is a
                // path-traversal/injection surface. The real name is kept only for
                // the activity log/description, never for the on-disk path.
                $extension = $file->getClientOriginalExtension();
                $filename = time().'_'.\Illuminate\Support\Str::random(8).($extension ? '.'.$extension : '');
